...
class BlogViewSetTests(TestCase):
    ...
    def test_get_blog_list_unauthenticated(self):
        """
        Unauthorized request should be able to get the list of blogs
        """
    ...
    def test_create_blog_authenticated(self):
        """Create a blog using authenticated user"""
        blog_data = {
            "title": "New Test Blog",
    ...
    def test_update_blog_authenticated(self):
        ...
        response = self.client1.put(
            reverse("blog-detail", args=[blog.id]), updated_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_blog_by_author_unauthenticated(self):
        """
        Search the blog by author name
        """
        self.create_blog()
    ...
    def test_get_blogs_by_date(self):
        """
        Search the blog by date
        """
        blog = self.create_blog()
        response = self.client1.get(
            reverse("blog-by-date") + f"?date={blog.created_at.date()}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
    ...
    def test_get_blogs_created_after_date(self):
        """
        Search the blog created after specific date
        """
        blog = self.create_blog()
        date_after = blog.created_at.date() - timedelta(days=1)
        response = self.client1.get(
            reverse("blog-created-after-date") + f"?date={date_after}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
    ...
    def test_get_blogs_created_before_date(self):
        """
        Search the blog created before specific date
        """
        blog = self.create_blog()
        date_before = blog.created_at.date() + timedelta(days=1)
        response = self.client1.get(
            reverse("blog-created-before-date") + f"?date={date_before}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
    ...
    def test_get_blogs_by_different_users(self):
        """
        Test that user is able to retrieve their own blogs only
        """
        self.create_blog(author=self.user1)
        self.create_blog(author=self.user2)

        # Retrieve blogs for user 1
        response_user1 = self.client1.get(
            reverse("blog-list") + f"?author={self.user1.id}"
        )

        # Retrieve blogs for user 2
        response_user2 = self.client2.get(
            reverse("blog-list") + f"?author={self.user2.id}"
        )
    ...
    def test_update_blog_by_different_users(self):
        """
        Test that one user is not allowed to update vlog created by someone else
        """
        ...
        response_user2 = self.client2.put(
            reverse("blog-detail", args=[blog_user1.id]), updated_data, format="json"
        )
        self.assertEqual(response_user2.status_code, status.HTTP_403_FORBIDDEN)

        response_user1 = self.client1.put(
            reverse("blog-detail", args=[blog_user1.id]), updated_data, format="json"
        )
        self.assertEqual(response_user1.status_code, status.HTTP_200_OK)
        self.assertEqual(Blog.objects.get().title, "Updated Test Blog")
